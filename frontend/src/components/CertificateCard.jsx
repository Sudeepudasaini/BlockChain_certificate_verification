    ...
    <div className="card-base hover:shadow-md">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <p className="text-sm text-slate-600 font-medium">{certificate.universityName}</p>
            <p className="text-xs text-slate-400">{new Date(certificate.issueDate).toLocaleDateString()}</p>
          </div>
        </div>
      ...
      {/* Student Name */}
      <h3 className="text-xl sm:text-2xl font-semibold text-slate-900 mb-3">{certificate.studentName}</h3>

      {/* Degree Info */}
      <div className="grid grid-cols-2 gap-4 mb-4 py-3 border-y border-gray-100">
        <div>
          <p className="text-xs text-slate-500">Degree</p>
          <p className="font-medium text-slate-900">{certificate.degree}</p>
        </div>
        <div>
          <p className="text-xs text-slate-500">Graduation Year</p>
          <p className="font-medium text-slate-900">{certificate.graduationYear}</p>
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
            <p className="font-mono text-sm text-slate-800 truncate break-words">{certificate.sha256Hash}</p>
        </div>
      </div>
      ...
      {showActions && (
        <div className="flex gap-3">
          <button onClick={handleDownload} className="flex-1 btn-primary text-sm">
            📥 Download
          </button>
          <button
